...
As demonstrated in the examples, the Jacobian functions do not have return values. Instead, they each have `JJ` inputs, which are pre-allocated 2D matrices (or 1D arrays) that you can fill within the functions. The shape (1D or 2D) is dictated by the choice for `linsolver`. See the full solver documentation for more information. 

The convention of filling the pre-allocated output memory (in this case `JJ`)
is a standard convention in scikit-SUNDAE. All user-defined functions follow this same pattern. In each user-defined function there is only one expected output array. All other arrays should be treated as read-only. So, in the case of the Jacobian function, trying to write to any of `y`, `yp`, or `res` will raise an error. Writing to `yp` in `rhsfn` and `res` in `resfn`, however, is allowed as they are the expected output arrays for those respective functions. The optional `userdata` argument must be present in ALL or NO user-defined functions. For example, we provide it in both expressions for the `IDA` example, but neither in the `CVODE` example. Even if `userdata` is only used in one user-defined function, it must be present in all signatures so the solver can handle them correctly. 

.. note:: 
    
    Take care to not overwrite any pre-allocated arrays. You should NEVER set the variable directly equal to a value, e.g., `JJ = ...`. Instead, ALWAYS use indices. Even if you can fill all values in `JJ` using vector math, the correct way to do this is to write out `JJ[:,:] = ...` (for 2D) or `JJ[:] = ...` (for 1D). This makes sure the pre-allocated matrix/array is filled rather than being overwritten. If you want to make sure the matrix starts out as zeros, use something like `JJ[:,:] = np.zeros_like(JJ)` (for 2D) or `JJ[:] = np.zeros_like(JJ)` (for 1D), and then fill in the non-zero indices.
